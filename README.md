...
We conducted a series of experiments to assess the performance of the pipeline: (1) evaluation of the GraphSAGE affinity predictor on test data and comparison to baseline models, (2) evaluation of the VGAE generative model’s ability to produce novel, valid, and diverse molecules, (3) a demonstration of the integrated generation+prediction loop by screening a large external library (ZINC20) using the GraphSAGE model, and (4) the latent space optimization results on selected candidates.

##### Affinity Prediction Performance: 
We first report the predictive accuracy of our GraphSAGE model on the held-out test set of ligand–target pairs from ChEMBL. As baselines, we implemented a classic fingerprint-based QSAR model (2048-bit Morgan fingerprint of the ligand + a one-hot encoding of the target protein, fed into a 3-layer feed-forward neural network) and a standard structure-based docking scoring. Our GraphSAGE model achieved an RMSE of 0.54 log units, substantially outperforming the fingerprint-based model (0.79) and also better than the docking scoring (0.68) on this dataset. The Pearson correlation of 0.82 indicates a strong linear relationship between predicted and true affinities across a range of potencies. Figure 2 illustrates the correlation: the blue points (predictions) lie close to the red line (ideal parity). The high ROC-AUC (0.94) means the model can almost perfectly rank actives above inactives. Notably, the enrichment factor EF@5% of 12.3 means that if we took the top 5% of compounds as predicted by GraphSAGE, they are ~12 times more likely to be true actives than a random selection – a crucial metric for virtual screening efficiency. In comparison, the baseline MLP had an EF of only ~5, indicating it was much less effective at early recognition of actives. Docking scoring achieved EF ~8 in our test, which GraphSAGE surpasses, consistent with literature that ML scoring functions can enrich actives more effectively by learning complex interaction patterns​.
...
The GraphSAGE affinity predictor achieved high accuracy in predicting binding affinities, with an RMSE below 0.6 log units and an ROC-AUC of ~0.94 on held-out data. It outperformed a traditional fingerprint-based approach, highlighting the benefit of learned graph representations and incorporating protein–ligand interaction information. The model’s ability to generalize to new scaffolds and its high enrichment factor indicate it is suitable as a virtual screening tool to prioritize candidates from large libraries.
- **RMSE:** 0.54  
- **Pearson R:** 0.82  
- **ROC-AUC:** 0.94  
- **EF@5%:** 12.3  
...
The VGAE generative model produced a vast diversity of molecules, most of which were novel. By sampling the latent space, we obtained thousands of drug-like structures that expand beyond the chemistry of the training set. Many of these generated molecules would not be obvious to human designers, underscoring the creative potential of deep generative models in proposing new scaffolds.
...
Efficient Virtual Screening: By applying the GraphSAGE model to ~1 million molecules from ZINC20, we rapidly identified a short-list of candidates likely to bind PD targets. This ML-driven screening is orders of magnitude faster than exhaustive docking and proved effective – the top-ranked hits were highly enriched in plausible binders (confirmed via docking and known SAR). Such a hybrid approach leverages the speed of ML and the physics awareness of docking to handle ultra-large chemical spaces (billions of compounds)​.

- Screened **1 million** molecules using GraphSAGE  
...
Molecule Optimization: Through latent space gradient ascent, we improved initial candidates to even higher predicted potencies. For instance, one candidate’s predicted IC50 improved from ~100 nM to ~10 nM after optimization. The structural modifications made during this process were coherent with medicinal chemistry intuition, which builds confidence in the method. The fact that the model autonomously added functional groups known to enhance binding (like halogens for better fit or polar groups for H-bonds) is notable. It suggests the model captured some underlying structure–activity relationships (SAR) from the data and can exploit them in novel compounds.
